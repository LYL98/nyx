...
Component({
  ...
  data: {
    query: {
      ...
      condition:'',
      custom_type:'B',
      sale_amount_unhandled: 1
    },
    operate: '',
    damagedLogTypes: Constant.DAMAGED_LOG_TYPES(),
    filterData: [
      {
        key: 'sale_type',
        title: '销售类型',
      ...
      },
      {
        key: 'type',
        title: '处理记录类型',
        items: [{key: 'offline_sale', title: '线下销售', active: true},{key: 'back_to_supply_for_sale', title: '退供代售', active: true}] //{key: '', title: '全部', active: true}
      },
      ],
  },
  //组件生命周期
...
filterChange(e){
  ...
  for(let j = 0;j < filterData[1].items.length; j++){
    if(filterData[1].items[j].key === query.type){
  ...
  getData(callback){
    ...
    that.setData({
      ...
      Http.get(Config.api.trashLogQuery, that.data.query).then(res => {
