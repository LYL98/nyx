  ...
  //组件生命周期
	lifetimes: {
      ...
      if(page && page.data.selectStorehouse){
        query.src_storehouse_id = page.data.selectStorehouse.id; //从首页取仓id
      }
  ...
  methods: {
    ...
    baseWarehouseList(){
      ...
      Http.get(Config.api.baseWarehouseList, {
        storehouse_id: query.src_storehouse_id,
        need_num: 50
      }).then(res => {
        let rd = res.data;
        rd.forEach(item => {
    ...
    getData(callback){
      ...
      that.setData({
        ...
        Http.get(Config.api.supDistributeOutPlanQuery, that.data.query).then(res => {
        ...
        this.setData({ query }, ()=>{
          this.getData((rd) => {
            if(rd.items.length === 1){
              wx.navigateTo({
                url: `/packageWarehouse/pages/out-storage-plan/distribute-out-storage/distribute-out-storage?index=0`
              });
            }
          });
        });
