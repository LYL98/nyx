import { Constant, Http, Config } from "./../../../../utils/index";
...
Component({
  ...
  data: {
    ...
    query: {
      custom_type: 'B',
      status: 'audit_success,part_in', //audit_success,part_in
      tar_storehouse_id: '',
      for_pre: 0, //1预采 0反采
    ...
    filterData: [{
      ...
      title: '选择状态',
      items: [
        {key: 'audit_success,part_in', title: '全部', active: true},
  ...
  methods: {
    ...
    filterChange(e){
      ...
      query.status = data.status || 'audit_success,part_in';
      query.page = 1;
    ...
    getData(callback){
      ...
      this.setData({
        ...
        Http.get(Config.api.fromSupplierOrderQuery, {
          ...query,
          is_mobile: query.status === 'audit_success,part_in' ? 1 : 0
        }).then(res => {
    ...
    skipDetail(e){
      ...
      if(auth.isAdmin || auth.ItemLocalPurchaseDetail){
