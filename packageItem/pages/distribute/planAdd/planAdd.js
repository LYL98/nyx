...
Page({
  ...
  addItem(){
    ...
    addData.p_items.push({num: '',c_item_id: ''});
    this.setData({ addData });
  },
  ...
  handleAffirm(){
    let { addData } = this.data;
    let con = true;
    if(!addData.src_storehouse_id){
      addData.src_storehouse_id_error = '请选择调出仓';
      con = false;
    }
    if(!addData.tar_storehouse_id){
      addData.tar_storehouse_id_error = '请选择调入仓';
      con = false;
    }
  ...
  supDistributePlanAdd(){
    ...
    that.setData({subLoading: true}, ()=>{
      Http.post(Config.api.supDistributePlanAdd, {
        ...addData,
        p_items: pItems
      }).then(res => {
